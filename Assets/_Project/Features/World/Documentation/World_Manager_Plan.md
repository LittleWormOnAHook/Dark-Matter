...
The **World Manager** is the Dark Matter: Genesis module responsible for **building, laying out, and activating** the playable Io surface — terrain, biomes, and all world content (enemies, resources, pets, echoes, quests, shards, POIs, hazards).

**Workflow in one sentence:** Designers assign prefabs and tune settings in a `WorldBuildProfile`; the World Manager procedurally builds geography (via a temporary external terrain authoring tool, then export), scatters gameplay content, and hands off runtime activation to existing systems.
...
### 2.2 What the World Manager owns
...
Assets/_Project/Features/World/
  ...
  Tests/
  Documentation/
    World_Manager_Plan.md          ← this file
```
...
Directors modulate **when** (danger budget, echo caps); World Manager defines **where**.
...
4. `World_Manager_Plan.md` (this document).
...
*Dark Matter Studios — Dark Matter: Genesis — World Manager Plan*  
