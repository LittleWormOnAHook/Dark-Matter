...
Shader "Burn Effect"
{
	Properties
	{
		...
		_BurnEffect("Burn Effect", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float2 panner2_g4 = ( ( _TimeParameters.x * 0.05 ) * float2( -1,0 ) + temp_cast_0);
				float2 uv_BurnEffect = packedInput.ase_texcoord5.xy * _BurnEffect_ST.xy + _BurnEffect_ST.zw;
				
				surfaceDescription.BaseColor = ( ( tex2D( _BurnMask, uv_BurnMask ) * tex2D( _FireTexture, panner2_g4 ) ) * ( tex2D( _BurnEffect, uv_BurnEffect ) * ( _TimeParameters.y + 1.5 ) ) ).rgb;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 temp_cast_0 = (0.0).xx;
				float2 panner2_g4 = ( ( _TimeParameters.x * 0.05 ) * float2( -1,0 ) + temp_cast_0);
				float2 uv_BurnEffect = packedInput.ase_texcoord2.xy * _BurnEffect_ST.xy + _BurnEffect_ST.zw;
		...
		Pass
		{
						...
						)
			{
				...
				float2 temp_cast_0 = (0.0).xx;
				float2 panner2_g4 = ( ( _TimeParameters.x * 0.05 ) * float2( -1,0 ) + temp_cast_0);
				float2 uv_BurnEffect = packedInput.ase_texcoord7.xy * _BurnEffect_ST.xy + _BurnEffect_ST.zw;
