...
namespace Project.Player
{
    ...
    public class PlayerDeathHandler : MonoBehaviour
    {
        ...
        public void Respawn()
        {
            ...
            CleanupDeathState();
            PioneerInvectorDeathRagdoll deathRagdoll = ResolveInvectorDeathRagdoll();
            // Capture belly/back while the corpse is still posed.
            deathRagdoll?.CaptureLieOrientation();
            ...
            deathRagdoll?.ResetForRespawn();
            // Retry: stand up where we died (not at scene spawn).
            Vector3 place = transform.position;
            Quaternion upright = transform.rotation;
            if (deathRagdoll != null && deathRagdoll.TryGetDeathPose(out Vector3 deathPos, out Quaternion deathRot))
            {
                place = deathPos;
                upright = deathRot;
            }
            ...
            place = SnapRetryFeetToGround(place);
            transform.SetPositionAndRotation(place, upright);
            if (character != null)
            ...
            // FullBody StandUp@FromBelly / FromBack after reset (matches death lie).
            deathRagdoll?.PlayStandUpAfterRespawn();
        }
        ...
        private static Vector3 SnapRetryFeetToGround(Vector3 approx)
        {
            Vector3 origin = approx + Vector3.up * 1.25f;
            if (Physics.Raycast(origin, Vector3.down, out RaycastHit hit, 4f, ~0, QueryTriggerInteraction.Ignore))
            {
                approx.y = hit.point.y;
                return approx;
            }
            return approx;
        }
