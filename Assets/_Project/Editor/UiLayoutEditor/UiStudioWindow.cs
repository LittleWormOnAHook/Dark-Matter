...
{
    /// <summary>
    /// UI Studio: browse panels, preview sandbox, edit layout profiles and scriptable styling.
    /// </summary>
    public partial class UiStudioWindow : EditorWindow
    {
        ...
        private bool hierarchyRebuildPending = true;
        private bool playModeRefreshScheduled;
        private bool showRectTransform = true;
        private bool showImage = true;
        private bool showRawImage = true;
        private bool showText = true;
        private bool showLegacyText = true;
        ...
        public static void ShowWindow()
        {
            ...
            window.TryResolveCanvasFromSelection(rebuildHierarchy: true);
        }
        ...
        {
            TryResolveCanvasFromSelection(rebuildHierarchy: true);
        ...
        private void OnPlayModeStateChanged(PlayModeStateChange state)
        {
            ...
            if (playModeRefreshScheduled)
                return;

            playModeRefreshScheduled = true;
            EditorApplication.delayCall += DeferredRefreshAfterPlayMode;
        }

        private void DeferredRefreshAfterPlayMode()
        {
            playModeRefreshScheduled = false;
            if (Application.isPlaying)
                return;
        ...
        private void OnGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("Assign a Root Canvas, open UI Preview Scene, or open the Inventory Slot prefab.", MessageType.Warning);
                return;
            }
            ...
            EditorGUILayout.BeginHorizontal(GUILayout.MinHeight(300f), GUILayout.ExpandHeight(true));
        ...
        private void DrawHierarchyPanel()
        {
            ...
            if (hierarchyRebuildPending && rootCanvas != null)
                RebuildFlatList();
            else if (flatList.Count == 0 && rootCanvas != null)
                RebuildFlatList();
        ...
        private void DrawInspectorPanel()
        {
            ...
            {
                showText = EditorGUILayout.Foldout(showText, "Text (TMP)", true);
                if (showText)
                    UiLayoutEditorInspectorDrawers.DrawTextSection(text);
            }
            else if (target.TryGetComponent(out Text legacyText))
            {
                showLegacyText = EditorGUILayout.Foldout(showLegacyText, "Text (Legacy)", true);
                if (showLegacyText)
                    UiLayoutEditorInspectorDrawers.DrawLegacyTextSection(legacyText);
            }
        ...
        private void TryResolveCanvasFromSelection(bool rebuildHierarchy = false)
        {
        ...
        private void EnsureRuntimeUiForHierarchy()
        {
            if (!Application.isPlaying || EditorApplication.isPlayingOrWillChangePlaymode)
                return;

            Object.FindAnyObjectByType<JournalPanelUI>()?.EnsureUiBuiltForLayoutEditor();
            EnemyLootDialogUI.EnsureBuiltForLayoutEditor();
        }
        ...
        {
            if (Application.isPlaying && panel.Category == "Journal")
            {
                Object.FindAnyObjectByType<JournalPanelUI>()?.EnsureUiBuiltForLayoutEditor();
            EnemyLootDialogUI.EnsureBuiltForLayoutEditor();
                RebuildFlatList();
        ...
        private void CapturePanelHierarchy(UiPanelDefinition panel, RectTransform panelRect)
        {
            ...
            if (panel.PanelId == UiPanelIds.InventoryPanel && IsEmbeddedInventoryPanel(panelRect))
            {
                Debug.LogWarning(
                    "[UI Studio] Skipped inventory capture while embedded in journal. Close the inventory tab first.");
                return;
            }
            ...
            if (!string.IsNullOrEmpty(panel.PanelId))
                UiStudioProfileIO.SaveFromPanelRoot(panelRect, panel.PanelId, includeHierarchy: true);

            Debug.Log($"[UI Studio] Captured panel \"{panel.Label}\" (scene + profile when PanelId set).");
        }
        ...
        private void CaptureVisiblePanels()
        {
            ...
            for (int i = 0; i < UiLayoutEditorPanelRegistry.Panels.Length; i++)
            {
                ...
                if (panel.PanelId == UiPanelIds.InventoryPanel && IsEmbeddedInventoryPanel(rect))
                {
                    skippedEmbedded++;
                    continue;
                }
                ...
                if (!string.IsNullOrEmpty(panel.PanelId))
                    UiStudioProfileIO.SaveFromPanelRoot(rect, panel.PanelId, includeHierarchy: true);
            ...
            if (skippedEmbedded > 0)
                message += $"\n\nSkipped {skippedEmbedded} embedded inventory capture — close the journal inventory tab and capture Inventory Panel directly for standalone layout.";

            EditorUtility.DisplayDialog("UI Studio", message, "OK");
        }

        private static bool IsEmbeddedInventoryPanel(RectTransform panelRect)
        {
            InventoryUI inventoryUi = panelRect.GetComponentInParent<InventoryUI>();
            return inventoryUi != null && inventoryUi.IsInventoryEmbedded;
        }
