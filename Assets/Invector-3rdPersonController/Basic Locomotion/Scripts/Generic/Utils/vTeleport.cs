...
public class vTeleport : MonoBehaviour
{
    ...
    {
        if (collider == null || targetPoint == null)
            return;
        ...
        TeleportPhaseEffect settings = GetComponent<TeleportPhaseEffect>();
        if (settings == null && targetPoint != null)
            settings = targetPoint.GetComponent<TeleportPhaseEffect>();

        TeleportPhaseEffect.PlayAt(teleporter, teleporter.position, settings);
    }
}